...
def _API용_기록_구성(대화_id: int, 이전_기록: list[dict]) -> list[dict]:
    """최근 20개는 그대로 보내고, 그보다 오래된 부분은 요약해 하나의 문맥 메시지로 압축한다.

    예전에는 20개보다 오래된 메시지를 그냥 잘라버려서 대화가 길어지면 초반 맥락이
    통째로 사라졌다. 이제는 잘리는 부분을 요약으로 남겨 계속 참조되게 한다. 매번 오래된
    부분 전체를 다시 요약하지 않고, 이미 요약에 반영된 개수(요약_메시지수)를 커서로 써서
    새로 "오래된" 취급을 받게 된 메시지만 기존 요약에 델타로 덧붙인다 — 대화가 아무리
    길어져도 이 요약 작업의 비용은 매 턴 일정하게 유지된다.
    """
    최근 = 이전_기록[-20:]
    오래된 = 이전_기록[:-20]
    if not 오래된:
        return [{"role": m["role"], "content": m["content"]} for m in 최근]

    기존_요약, 요약_메시지수 = repo.대화_요약_불러오기(대화_id)
    if 요약_메시지수 < len(오래된):
        새로_요약할_부분 = 오래된[요약_메시지수:]
        기존_요약 = ai_agent.대화_요약_생성(기존_요약, 새로_요약할_부분)
        repo.대화_요약_저장(대화_id, 기존_요약, len(오래된))

    return [
        {"role": "user", "content": f"[이전 대화 요약 — 화면에는 안 보이지만 참고할 맥락]\n{기존_요약}"},
        {"role": "assistant", "content": "확인했습니다. 이어서 답변하겠습니다."},
...
def _제안_요약(제안: dict, 전체_df: pd.DataFrame) -> dict:
    ...
    if 유형 == "propose_restore_diagnosis_layer":
        버전_id = 인자.get("버전_id")
        버전 = repo.제조AI진단_레이어_버전_조회(버전_id)
        if not 버전:
            return {"유형": 유형, "오류": f"버전#{버전_id}을 찾을 수 없습니다."}
        코드 = 버전["코드"]
        현재 = repo.제조AI진단_레이어_조회(코드)
        if not 현재:
            return {"유형": 유형, "오류": f"'{코드}' 레이어를 찾을 수 없습니다."}
        return {
            "유형": 유형,
            "대상id": None,
            "제목": f"{코드} {현재.get('이름', '')} — 버전#{버전_id}으로 되돌리기",
            "변경": [
                {"필드": 필드, "이전값": 현재.get(필드), "새값": 버전.get(필드)}
                for 필드 in ("이름", "설명", "ai개입지점", "수준들")
            ],
        }
...
def _제안_반영(제안: dict, 전체_df: pd.DataFrame, 작성자: str = "AI채팅") -> None:
    ...
    if 유형 == "업로드":
        ...
        repo.제조AI진단_레이어_수정(인자.get("코드"), 인자.get("변경필드", {}), 작성자=작성자)
    elif 유형 == "propose_delete_diagnosis_layer":
        repo.제조AI진단_레이어_삭제(인자.get("코드"))
    elif 유형 == "propose_restore_diagnosis_layer":
        버전 = repo.제조AI진단_레이어_버전_조회(인자.get("버전_id"))
        if 버전:
            repo.제조AI진단_레이어_수정(
                버전["코드"],
                {"이름": 버전["이름"], "설명": 버전["설명"], "ai개입지점": 버전["ai개입지점"], "수준들": 버전["수준들"]},
                작성자=작성자,
            )
