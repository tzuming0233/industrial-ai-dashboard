...
# DB 접근/파일파싱 로직은 backend/app/ 로 옮겨서, 나중에 만들 FastAPI 백엔드와
# 이 Streamlit 앱이 완전히 같은 함수를 공유하게 한다(로직이 두 곳에서 따로 놀며
# 어긋나는 걸 막는 것이 이 분리의 유일한 목적 — 옮기며 로직을 바꾸지 않았다).
from backend.app.repository import (
    금액_컬럼들, 편집_컬럼순서, 사업단계_옵션, 주관참여구분_옵션,
    DB_준비, 채팅_DB_준비, 사업현황_컬럼_보강, 연간목표_DB_준비, 연간목표_불러오기, 연간목표_저장,
    투입인력_DB_준비, 투입인력_불러오기, 투입인력_저장, 투입인력_삭제, 이력_DB_준비, 온톨로지_DB_준비,
    사업현황_불러오기, 사업현황_저장, 이력_불러오기, 전체_이력_불러오기, 이력_저장,
    대화_목록_불러오기, 대화_생성, 대화_제목_설정, 대화_삭제, 채팅기록_불러오기, 채팅기록_저장,
    온톨로지_노드_불러오기, 온톨로지_관계_불러오기, 온톨로지_관계_추가, 온톨로지_관계_삭제,
    온톨로지_초기화, 온톨로지_관계_직접추가, 온톨로지_검색,
)
from backend.app.files import (
    엑셀로_변환, _업로드_원본_읽기, _pdf_텍스트_추출, _hwp_텍스트_추출, _LLM_매핑_적용,
    _제안_기본값, _제안_추가행들,
)
...
def _스타일_적용() -> None:
    st.markdown(
        ...
        /* 채팅 목록을 담은 세로 flex 컨테이너의 기본 align-items:center 때문에 말풍선 폭을
           ...
           (키 이름의 한글 "채팅_상자"는 실제 DOM에서 "st-key---_--"로 깨져서 남는다 —
           브라우저에서 직접 확인한 값으로 지정. data-test-scroll-behavior는 메시지 유무에
           따라 "normal"/"scroll-to-bottom"으로 바뀌어서 대신 이 클래스를 쓴다.) */
        .st-key---_--[data-testid="stVerticalBlock"] {{
            align-items: stretch !important;
        }}
        /* align-items:stretch를 줘도 각 메시지 행(stLayoutWrapper)이 자체적으로
           콘텐츠 크기만큼만 폭을 차지하고 있어 늘어나지 않았다 — 폭 100%를 직접 강제한다. */
        [data-testid="stLayoutWrapper"]:has(> [data-testid="stChatMessage"]) {{
            width: 100% !important;
        }}
        ...
        @media (max-width: 640px) {{
            ...
            .st-key---_--[data-testid="stVerticalBlock"] {{ height: 380px !important; }}
        }}
        </style>
        """,
        unsafe_allow_html=True,
