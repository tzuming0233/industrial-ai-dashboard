...
TOOLS = [
    {
        ...
        "description": (
            ...
            "사업구분, 구분(신규/이월), 종료일 범위로 필터링할 수 있으며, "
...
def 조회_사업현황(사업구분=None, 구분=None, 종료일_이전=None, 종료일_이후=None) -> list[dict]:
    ...
    쿼리 = f"""
        SELECT 구분, 업체명, 용역명, 사업구분, 시작일, 종료일,
               계약금액, 기수입금액, 당해년도수입금액
        FROM 사업현황
        {where절}
        ORDER BY 종료일
...
def 질의하기(question: str, history: list[dict] | None = None, api_key: str | None = None) -> str:
    ...
    if not key:
        return "ANTHROPIC_API_KEY 환경변수가 설정되어 있지 않습니다. .env 파일을 확인하세요."
