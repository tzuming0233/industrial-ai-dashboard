...
TOOLS = [
    {
        ...
        "description": (
            ...
            "사업구분, 구분(신규/이월), 진행상태, 담당자(PM), 종료일 범위로 필터링할 수 있으며, "
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "진행상태": {
                    "type": "string",
                    "description": "예: RFP접수, 제안서작성, 제출, 평가, 수행, 완료, 보류",
                },
...
def 조회_사업현황(
    사업구분=None, 구분=None, 진행상태=None, 담당자=None, 종료일_이전=None, 종료일_이후=None
) -> list[dict]:
    ...
    쿼리 = f"""
        SELECT 구분, 업체명, 용역명, 사업구분, 담당자, 진행상태, 진행률, 시작일, 종료일,
               계약금액, 기수입금액, 당해년도수입금액
        FROM 사업현황
        {where절}
        ORDER BY 종료일
